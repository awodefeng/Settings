...
public final class DeviceProfilesSettings extends SettingsPreferenceFragment
    ...
    private static final String KEY_CONNECTION_STATES = "connection_states";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mConnectionStatesPref = (Preference) findPreference(KEY_CONNECTION_STATES);
        mUnpairPref = (Preference) findPreference(KEY_UNPAIR);
        if (device == null) {
    ...
    private void showOrHideProfileGroup() {
        ...
        if (!mProfileGroupIsRemoved && numProfiles == 0) {
            //there is no profile supported, the mConnectionStatesPref
            getPreferenceScreen().removePreference(mConnectionStatesPref);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen screen, Preference preference) {
        ...
        if (key.equals(KEY_UNPAIR)) {
            ...
            finish();
            return true;
        } else if (key.equals(KEY_CONNECTION_STATES)) {
    ...
    private void refresh() {
        ...
        if (SUPPORT_SMARTFREN_FEATURE) {
            int titleId = getConnectionTitle();
            mConnectionStatesPref.setTitle(titleId);
        }

        if (summaryResId != 0) {
            mConnectionStatesPref.setSummary(summaryResId);
        } else {
            mConnectionStatesPref.setSummary(null);
        }
        mConnectionStatesPref.setEnabled(!mCachedDevice.isBusy());
        mUnpairPref.setEnabled(!mCachedDevice.isBusy());
        refreshProfiles();
    }
    ...
    private int getConnectionTitle() {
        ...
        return profileConnected ? R.string.connection_states_disconnect : R.string.connection_states_connect;
    }
