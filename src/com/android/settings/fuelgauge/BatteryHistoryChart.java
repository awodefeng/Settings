...
public class BatteryHistoryChart extends View {
    ...
    private void updateTextPaintAttr() {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        /* SPRD: Modify 20140513 Spreadst of Bug 311268, Battery operating time display abnormal @{ */
        updatePaint4TextLen();
        updateTextPaintAttr();
        if (mLargeMode) {
        ...
        if (mLargeMode) {
            // SPRD: just for draw text position, Bug 274524
            int newTextDescent = mTextDescent * 2 + mTextDescent / 3;

            if (mHavePhoneSignal) {
                canvas.drawText(mPhoneSignalLabel, textStartX,
                        height - mPhoneSignalOffset - newTextDescent, mTextPaint);
            }
            if (mHaveGps) {
                canvas.drawText(mGpsOnLabel, textStartX,
                        height - mGpsOnOffset - newTextDescent, mTextPaint);
            }
            if (mHaveWifi) {
                canvas.drawText(mWifiRunningLabel, textStartX,
                        height - mWifiRunningOffset - newTextDescent, mTextPaint);
            }
            canvas.drawText(mWakeLockLabel, textStartX,
                    height - mWakeLockOffset - newTextDescent, mTextPaint);
            canvas.drawText(mChargingLabel, textStartX,
                    height - mChargingOffset - newTextDescent, mTextPaint);
            canvas.drawText(mScreenOnLabel, textStartX,
                    height - mScreenOnOffset - newTextDescent, mTextPaint);
    ...
    private void updatePaint4TextLen() {
        Rect rect = new Rect();
        mTextPaint.getTextBounds(mDurationString, 0, mDurationString.length(), rect);
        int actualWitdh = rect.right - rect.left;
        while (actualWitdh >= getWidth() - 10) {
            float paintSize = mTextPaint.getTextSize();
            mTextPaint.setTextSize(paintSize - 1);
            mTextPaint.getTextBounds(mDurationString, 0, mDurationString.length(), rect);
            actualWitdh = rect.right - rect.left;
        }
    }
}
