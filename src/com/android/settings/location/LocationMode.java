...
public class LocationMode extends LocationSettingsBase
    ...
    private static final boolean SUPORT_CMCC = SystemProperties.get("ro.operator").equals("cmcc");
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        refreshLocationMode();
        //gps disabled remove gps location mode
        if (WCN_DISABLED || !LOCATION_GPS_ENABLED) {
            root.removePreference(mHighAccuracy);
            root.removePreference(mSensorsOnly);
        }
        //remove agps config ui when gps disable and version not cmcc
        if (!SUPORT_CMCC || WCN_DISABLED ||!LOCATION_GPS_ENABLED){
