            ...
            if (storageState.equals(Environment.MEDIA_MOUNTED)) {
                ...
                if(file.exists()){
                    ...
                    if (levelPower >= 35) {
                        ...
                        try {
                            RecoverySystem.installPackage(cont, file);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } else {
                        Toast.makeText(cont, R.string.recovery_update_level, Toast.LENGTH_LONG).show();
                    }
                } else {
                    Toast.makeText(cont, R.string.recovery_no_package, Toast.LENGTH_LONG).show();
                }
            }else {
                // sdcard 不可用
                Toast.makeText(cont, R.string.recovery_sdacrd_status, Toast.LENGTH_LONG).show();
            }
        }
    }
