    ...
    */
    ...
    public void sortImpl(File file, String fileType)
    {
        ...
        if(file != null && file.exists())
        {
            ...
            mFileNameList = getSortedFileNameArray(file, mFileNameArray);
            // SPRD: if fileName startWith "." ,remove it
            for (int i = mFileNameList.size()-1; i>=0; i--) {
                if (mFileNameList.get(i).startsWith(".")) {
                    mFileNameList.remove(i);
                // SPRD: add for if sd card is not exist,dont show sdcard0
                } else if("sdcard0".equals(mFileNameList.get(i))
